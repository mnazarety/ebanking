...
if (isset($_POST['deposit'])) {
    ...
    /*
            ...
            */
    $result = "SELECT SUM(transaction_amt) FROM  tbl_transactions  WHERE account_id=?";
    ...
    if ($transaction_amt > $amt) {
        ...
        $query = "INSERT INTO tbl_transactions (tr_code, account_id, acc_name, account_number, acc_type,  tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone, receiving_acc_no, receiving_acc_name, receiving_acc_holder) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $notification = "INSERT INTO  tbl_notifications (notification_details) VALUES (?)";
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class=" col-md-4 form-group">
                                                    ...
                                                    <select name="receiving_acc_no" onChange="geteBankAccs(this.value);" required class="form-control">
                                                        <option>Select Receiving Account</option>
                                                        <?php
                                                        //fetch all iB_Accs
                                                        $ret = "SELECT * FROM tbl_bankaccounts WHERE client_id !='".$client_id."'";
                                                        $stmt = $mysqli->prepare($ret);
                                                        $stmt->execute(); //ok
                                                        $res = $stmt->get_result();
                                                        $cnt = 1;
