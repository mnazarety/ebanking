...
if (isset($_POST['reset_password'])) {
  ...
  $checkEmail = mysqli_query($mysqli, "SELECT `email` FROM `tbl_clients` WHERE `email` = '" . $_POST['email'] . "'") or exit(mysqli_error($mysqli));
  if (mysqli_num_rows($checkEmail) > 0) {

    $n = date('y');
    $new_password = bin2hex(random_bytes($n));
    //Insert Captured information to a database table
    $query = "UPDATE tbl_clients SET  password=? WHERE email =?";
